...
class exit_interview_db(models.Model):
    ...
    def check_and_update_status(self):
        if self.scheduled_date < timezone.now().date():
            self.status = "Expired"
        elif self.status == "Expired" and self.scheduled_date >= timezone.now().date():
            self.status = "Pending"
...
class OjtAssessment(models.Model):
    ...
    def check_and_update_status(self):
        if self.dateAccepted < timezone.now().date():
            self.status = "Expired"
        elif self.status == "Expired" and self.dateAccepted >= timezone.now().date():
            self.status = "Pending"
