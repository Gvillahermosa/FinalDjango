...
from .models import studentInfo, RequestedGMC,Equipment,Schedule,ProcurementItem,Storage,ExcelData, BorrowingRecord
...
from .models import TestArray, studentInfo, counseling_schedule, exit_interview_db, OjtAssessment, IndividualProfileBasicInfo, IntakeInverView, GuidanceTransaction
import json
import openpyxl
import pandas as pd
...
def individualProfile(request):
    ...
    if user. user_type == 'sao admin':
        return render(request, 'guidance/admin/access_violation.html',{'info': 'For student use only.'})
    if request.method == 'POST':
...
def intake_interview_view(request):
    ...
    if not user. user_type == 'sao admin':
        return render(request, 'guidance/user/access_violation.html',{'info': 'For admin user only!'})
...
def individual_profile_sheet(request, id):
    ...
    return render(request, 'guidance/admin/individual_profile_sheet.html', {'form': intake})
def search_student_info_for_intake(request):
     if request.method == 'POST':
        id_number = request.POST.get('id_number', '')
        try:
            student = studentInfo.objects.get(studID = id_number)
            individual = IndividualProfileBasicInfo.objects.filter(studentId = student).order_by('-dateFilled')
            items = []
...
def counseling_app_admin_view(request):
    ...
    if not user. user_type == 'sao admin':
        return render(request, 'guidance/user/access_violation.html',{'info': 'For admin user only!'})
    now = timezone.now()
    meeting_requests = counseling_schedule.objects.filter(scheduled_date = now).select_related('studentID').order_by('-dateRecieved')
...
def counselor_scheduler(request):
    if request.method == 'POST':
        ...
        status = "Accepted"
        counseling = counseling_schedule(counselingOrigin = 'counselor',dateRecieved = now, studentID = student, reason = reason, scheduled_date = date_scheduled,status= status, scheduled_time = time, email='')
        counseling.save()
        transaction = GuidanceTransaction(
...
def exit_interview(request):
    ...
    if user.student_id:
        student_info = user.student_id
    return render(request, 'guidance/user/exit_interview.html',{'form': form , 'student_id': student_info.studID})
@login_required
def exit_interview_admin_view(request):
    ...
    if not user. user_type == 'sao admin':
        return render(request, 'guidance/user/access_violation.html',{'info': 'For admin user only!'})
    now = timezone.now()
    exit_interview_request = exit_interview_db.objects.filter(scheduled_date = now).select_related('studentID').order_by('-dateRecieved')
...
def ojt_assessment(request):
    ...
    if user.student_id:
        student_info = user.student_id
    return render(request,'guidance/user/ojt_assessment.html',{'form': form , 'student_id': student_info.studID})
@login_required
...
def guidance_transaction(request):
    ...
    if not user. user_type == 'sao admin':
        return render(request, 'guidance/user/access_violation.html',{'info': 'For admin user only!'})
    now = timezone.now()
    transactions = GuidanceTransaction.objects.filter(transactionDate = now).order_by('-transactionId')
...
def daily_montly_guidance_transaction(request):
    if request.method == 'POST':
        ...
        if(transactionFrom == 'all'):
            if(sorttype == 'all'):
                transactions = GuidanceTransaction.objects.all().order_by('-transactionId')
                return render(request, 'guidance/admin/transaction.html', {'form': transactions})
            elif(sorttype == 'monthly'):
                now = timezone.now()    
                current_month = now.month
                transactions = GuidanceTransaction.objects.filter(transactionDate__month = current_month).order_by('-transactionId')
                return render(request, 'guidance/admin/transaction.html', {'form': transactions})
            elif(sorttype == 'daily'):
                now = timezone.now()
                transactions = GuidanceTransaction.objects.filter(transactionDate = now).order_by('-transactionId')
            ...
            if(sorttype == 'all'):
                transactions = GuidanceTransaction.objects.filter(transactionOrigin = transactionFrom).order_by('-transactionId')
                return render(request, 'guidance/admin/transaction.html', {'form': transactions})
            elif(sorttype == 'monthly'):
                now = timezone.now()    
                current_month = now.month
                transactions = GuidanceTransaction.objects.filter(transactionOrigin = transactionFrom, transactionDate__month = current_month).order_by('-transactionId')
                return render(request, 'guidance/admin/transaction.html', {'form': transactions})
            elif(sorttype == 'daily'):
                now = timezone.now()
                transactions = GuidanceTransaction.objects.filter(transactionOrigin = transactionFrom, transactionDate = now).order_by('-transactionId')
                return render(request, 'guidance/admin/transaction.html', {'form': transactions})

def show_transaction_specific_date(request):
    if request.method == 'POST':
        selectedDate = request.POST.get('selectedDate', '')
        transactions = GuidanceTransaction.objects.filter(transactionDate = selectedDate).order_by('-transactionId')
        return render(request, 'guidance/admin/transaction.html', {'form': transactions})
...
def sort_counseling_app_admin_view(request):
    ...
    if(sortBy == 'all' and sortStatus == 'All'):
        meeting_requests = counseling_schedule.objects.all().select_related('studentID').order_by('-dateRecieved')
        ...
        if(sortBy == 'all'):
            meeting_requests = counseling_schedule.objects.filter(status = sortStatus).select_related('studentID').order_by('-dateRecieved')
            time = {
                '8-9': '8:00 AM - 9:00 AM',
                '9-10': '9:00 AM - 10:00 AM',
                '10-11': '10:00 AM-11:00 AM',
            ...
            if(sortStatus == 'All'):
                ...
                meeting_requests = counseling_schedule.objects.filter(scheduled_date = now,status = sortStatus).select_related('studentID').order_by('-dateRecieved')
            time = {
                '8-9': '8:00 AM - 9:00 AM',
                '9-10': '9:00 AM - 10:00 AM',
                '10-11': '10:00 AM-11:00 AM',
            ...
            now = timezone.now()
            if(sortStatus == 'All'):
                meeting_requests = counseling_schedule.objects.filter(dateRecieved = now).select_related('studentID').order_by('-dateRecieved')
            else:
                meeting_requests = counseling_schedule.objects.filter(dateRecieved = now,status = sortStatus).select_related('studentID').order_by('-dateRecieved')
            time = {
                '8-9': '8:00 AM - 9:00 AM',
                '9-10': '9:00 AM - 10:00 AM',
                '10-11': '10:00 AM-11:00 AM',
...
def sort_exit_interview_admin_view(request):
    ...
    if(sortBy == 'all' and sortStatus == 'All'):
        now = timezone.now()
        exit_interview_request = exit_interview_db.objects.all().select_related('studentID').order_by('-dateRecieved')
        context = {'exit_interview_request': exit_interview_request,}
        return render(request, 'guidance/admin/exit_interview_admin.html', context)
    else:
        if(sortBy == 'all'):
            now = timezone.now()
            exit_interview_request = exit_interview_db.objects.filter(status = sortStatus ).select_related('studentID').order_by('-dateRecieved')
            context = {'exit_interview_request': exit_interview_request,}
            return render(request, 'guidance/admin/exit_interview_admin.html', context)
        elif(sortBy == 'scheduledToday'):
            now = timezone.now()
            if(sortStatus == 'All'):
                ...
                exit_interview_request = exit_interview_db.objects.filter(scheduled_date = now, status = sortStatus ).select_related('studentID').order_by('-dateRecieved')
            context = {'exit_interview_request': exit_interview_request,}
            return render(request, 'guidance/admin/exit_interview_admin.html', context)
        elif(sortBy == 'recievedToday'):
            now = timezone.now()
            if(sortStatus == 'All'):
                exit_interview_request = exit_interview_db.objects.filter(dateRecieved = now ).select_related('studentID').order_by('-dateRecieved')
            else:
                exit_interview_request = exit_interview_db.objects.filter(dateRecieved = now, status = sortStatus ).select_related('studentID').order_by('-dateRecieved')
            context = {'exit_interview_request': exit_interview_request,}
            return render(request, 'guidance/admin/exit_interview_admin.html', context)
