...
def upload_student_data(request):
    if request.method == "POST":
        if "excel_file" in request.FILES:
            ...
            if "student-profile" in excel_file.name and excel_file.name.endswith(
                (".xlsx", ".xls")
            ):
                ...
                messages.error(
                    request,
                    "Invalid file format or name. Please upload an Excel file with 'student-profile' in the filename.",
                )
                return redirect("studentLife_system:upload_student_data")
        else:
...
def update_return_status(request):
    if request.method == "POST":
        try:
            ...
            if not record_id:
                return JsonResponse(
                    {"status": "error", "message": "Record ID not provided"}, status=400
                )
            ...
            record.save()
            return JsonResponse(
                {"status": "success", "date_returned": record.date_returned}
            )
        except BorrowingRecord.DoesNotExist:
            return JsonResponse(
                {"status": "error", "message": "Record not found"}, status=404
            )
        except Exception as e:
...
def processed_gmc_transactions(request):
    ...
    return render(
        request,
        "adminUser/transactionsGMC.html",
...
def update_schedule(request, schedule_id):
    if request.method == "POST":
        ...
        if start_datetime and end_datetime:
            ...
            return JsonResponse(
                {"status": "error", "message": "Invalid data provided."}, status=400
            )
    else:
        return JsonResponse(
...
def equipmentTracker(request):
    ...
    if users.is_authenticated:
        ...
        try:
            student = studentInfo.objects.get(user_student=users)
        except studentInfo.DoesNotExist:
            student = None
...
def addEquipment(request):
    if request.method == "POST":
        ...
        if equipment_name and serial:
            ...
            messages.error(
                request, "Please provide both equipment name and serial number"
            )
        ...
        if student_id and equipment_id and date_borrowed:
            try:
                ...
                BorrowingRecord.objects.create(
                    student=student, equipment=equip, date_borrowed=date_borrowed
                )
                messages.success(
                    request, "Equipment borrowing record saved successfully"
                )
            except studentInfo.DoesNotExist:
                messages.error(request, "Student not found")
        ...
        try:
            ...
            if new_status == "delivered":
                ...
                storage = Storage.objects.create(
                    procurement_item=item, serial_no=serial_no
                )

                # Also create a new equipment entry
                equipment_name = (
                    item.item
                )  # Assuming 'item' is the name of the equipment
                equipmentSN = serial_no if serial_no else "No Serial Number"
                Equipment.objects.create(
                    equipmentName=equipment_name, equipmentSN=equipmentSN
                )
...
def display_storage_items(request):
    ...
    return render(
        request,
        "adminUser/ppmpTracker/display_storage_items.html",
...
def update_serial_no(request, storage_id):
    ...
    return render(
        request,
        "adminUser/ppmpTracker/update_serial_no.html",
        {"form": form, "storage_item": storage_item},
    )
...
def purchased_items(request):
    ...
    total_cost_sum = items.aggregate(
        total_cost_sum=Sum(F("quantity") * F("unit_price"))
    )["total_cost_sum"]

    return render(
        request,
        "adminUser/ppmpTracker/purchased_items.html",
        {"items": items, "total_cost_sum": total_cost_sum},
    )
...
def edit_excel_data(request):
    ...
    return render(
        request,
        "adminUser/ppmpTracker/edit_excel_data.html",
...
def update_excel_data(request, pk):
    ...
    return render(
        request, "adminUser/ppmpTracker/update_excel_data.html", {"form": form}
    )
...
def graduateTracer(request):
    user = request.user.is_staff
    return render(request, "alumni/users/graduateTracer.html", {"user": user})
        ...
        if student_id:
            try:
                ...
                if graduateForm.objects.filter(student__studID=student_id).exists():
                    ...
                    return render(
                        request, "alumni/users/graduateTracer.html", {"user": user}
                    )

                return render(
                    request,
                    "alumni/users/graduateTracer.html",
                    {"alumni": alumni_obj, "user": user},
                )
...
def alumni_events(request):
    ...
    return render(
        request, "alumni/users/alumni_events.html", {"events": events, "user": user}
    )
...
def jobfairs(request):
    ...
    return render(
        request, "alumni/users/jobfairs.html", {"job_fairs": job_fairs, "user": user}
    )


def yearbook(request):
    user = request.user.is_staff
    return render(request, "alumni/users/yearbook.html", {"user": user})
        ...
        if first_name and last_name:
            try:
                ...
                return render(
                    ...
                    {"yearbook_entry": yearbook_entry, "user": user},
                )
            except Yearbook.DoesNotExist:
                return render(
                    request,
                    "alumni/users/yearbook.html",
                    {"error_message": "No yearbook entry found.", "user": user},
                )
        else:
            return render(
                request,
...
def approve_alumni_request(request, alumni_id):
    ...
    if request.method == "POST":
        ...
        return redirect("studentLife_system:admin_idRequest", {"user": user})
...
def admin_transactionreport(request):
    ...
    report_type = request.GET.get(
        "report_type"
    )  # New parameter for report type selection
    ...
    if report_type == "projects":
        ...
        return render(
            request,
            "studentorg/ADMIN/transaction_report.html",
            {
                "projects": projects,
        ...
        total_amount_financial_statements = sum(
            statement.amount for statement in financial_statements
        )
        return render(
            request,
            "studentorg/ADMIN/transaction_report.html",
        ...
        total_financial_transactions = (
            financial_statements.count() + projects.count() + accreditations.count()
        )
...
def register_officer(request):
    if request.method == "POST":
        ...
        try:
            ...
            messages.error(
                request, "Officer with the provided student ID does not exist."
            )
        ...
        if officer.organization != organization:
            messages.error(
                request,
                "The provided organization does not match the officer's organization.",
            )
            return render(request, "studentorg/ADMIN/registerofficer.html")

        if officer.status != "approved":
            messages.error(
                request, "Officer status must be approved to create an account."
            )
            return render(request, "studentorg/ADMIN/registerofficer.html")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                request.session["officer_student_id"] = (
                    officer.student_id.student_id
                )  # Correct reference to student_id
                ...
                if officer.organization == "FSTLP":
                    ...
                    return redirect(
                        "studentLife_system:officer_login"
                    )  # Fallback redirect to login if organization is invalid
...
def admin_manageofficer(request):
    ...
    return render(
        request,
        "studentorg/ADMIN/admin_manageofficer.html",
        {"officers": reversed(officers)},
    )
...
def admin_manageadviser(request):
    ...
    return render(
        request,
        "studentorg/ADMIN/admin_manageadviser.html",
        {"advisers": reversed(advisers)},
    )
...
def admin_manageproject(request):
    ...
    return render(
        request,
        "studentorg/ADMIN/admin_manageproject.html",
...
def admin_managefinancial(request):
    ...
    if request.method == "POST":
        ...
        statement = get_object_or_404(
            FinancialStatement, financial_id=financial_id
    ...
    return render(
        request,
        "studentorg/ADMIN/admin_managefinancial.html",
...
def admin_manage_accreditations(request):
    ...
    if request.method == "POST":
        ...
        action = request.POST.get("action")
        accreditation = get_object_or_404(
            Accreditation, accreditation_id=accreditation_id
        )
        ...
        if action == "approve":
            ...
            if organization == "FSTLP":
                ...
                return redirect(
                    "studentLife_system:admin_manage_accreditations"
    ...
    return render(
        request,
        "studentorg/ADMIN/manage_accreditation.html",
        {"accreditations": reversed(accreditations)},
    )
...
def admin_view_accreditations(request):
    ...
    return render(
        request,
        "studentorg/ADMIN/view_accreditation.html",
        {"accreditations": approved_accreditations},
    )
...
def FSTLP_financial(request):
    ...
    return render(
        request, "studentorg/FSTLP/FSTLP_financial_statement.html", {"form": form}
    )
